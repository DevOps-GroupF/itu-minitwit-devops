...
To configure and execute our CI/CD pipeline we use GitHub Action, because we were already using GitHub for our repository, and GH Actions provided a seamless integration in our workflow in GitHub. %Furthermore, it was easy to set up, and the free plan was more than enough for our needs.
...
\begin{enumerate}
	...
	\begin{itemize}
		\item In our VM we have an used called deploy. When someone logs in as this used, either by SSH or by using su, a script is executed. %The person logging in cannot execute any other command other than the script, to prevent security issues should someone obtain the keys to the user. 
